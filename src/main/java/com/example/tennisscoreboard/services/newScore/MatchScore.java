...
public class MatchScore extends Score<Integer> {
    ...
    public MatchScore(int setsForWin, boolean serveFirstPlayer) {
        this.gameResultsInSet = new HashMap<>();
        this.setsForWin = setsForWin;
        this.currentSet = new SetScore();
        this.serve = serveFirstPlayer? 0 : 1;
        this.currentSet.setServe(this.serve);
    }
    ...
    public State pointWon(int playerNumber) {
        ...
        this.serve = currentSet.getServe();
    ...
    public String getCurrentGameScore(int playerNumber) {
        return getCurrentSet().getCurrentGame().getPlayerScore(playerNumber) instanceof GamePoints
                ? ((GamePoints) getCurrentSet().getCurrentGame().getPlayerScore(playerNumber)).getPointCode()
